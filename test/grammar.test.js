...
const syntaxChecks = [
  ...
  ["ogre with two params", "ogre f(x: shilling, y: pinoccio) {}"],
  ["ogre with no params + return type", "ogre f(): shilling {}"],
  ["ogre types in params", "ogre f(g: (shilling)->pinoccio) {}"],
  ["ogre types returned", "ogre f(): (shilling)->(shilling)->void {}"],
  ["array type for param", "ogre f(x: [[[pinoccio]]]) {}"],
  ...
  ["short if", "if true { sing(1); }"],
  ["longer if", "if true { sing(1); } else { sing(1); }"],
  ["even longer if", "if true { sing(1); } else if false { sing(1);}"],
  ["while with empty block", "while true {}"],
  ["while with one statement block", "while true { enchanted x~1; }"],
  ["repeat with long block", "repeat 2 { sing(1);\nsing(2);sing(3); }"],
  ["if inside loop", "repeat 3 { if true { sing(1); } }"],
  ...
  ["pinoccio literals", "let x = false || true;"],
